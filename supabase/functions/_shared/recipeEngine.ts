...
const MEDIUM_POOL: TaggedOption[] = [
  ...
  // Famous artists
  { text: 'Van Gogh Starry Night style, swirling thick brushstrokes, vivid blues and yellows', axes: { realism: 'low', complexity: 'high', energy: 'high', color_warmth: 'high', brightness: 'low' } },
  { text: 'Picasso cubist style, fragmented geometric faces, multiple perspectives', axes: { realism: 'low', complexity: 'high', energy: 'high', brightness: 'low', color_warmth: 'low' } },
  { text: 'Monet impressionist, soft water lilies, dappled light, dreamy blur', axes: { realism: 'low', brightness: 'high', energy: 'low', color_warmth: 'high', complexity: 'low' } },
  { text: 'Frida Kahlo surrealist style, lush flowers, vivid symbolic colors, folk art motifs', axes: { realism: 'low', complexity: 'high', color_warmth: 'high', energy: 'low', brightness: 'high' } },
  { text: 'Banksy street art, stencil graffiti, political irony, concrete wall', axes: { realism: 'high', energy: 'high', brightness: 'low', complexity: 'low', color_warmth: 'low' } },
  { text: 'Gustav Klimt gold leaf style, ornate patterns, Byzantine mosaic influence', axes: { complexity: 'high', color_warmth: 'high', brightness: 'high', realism: 'low', energy: 'low' } },
  { text: 'Edward Hopper lonely realism, empty diners, long shadows, isolation', axes: { realism: 'high', energy: 'low', brightness: 'low', color_warmth: 'high', complexity: 'low' } },
  { text: 'Keith Haring bold outlines, dancing figures, primary colors, street art', axes: { realism: 'low', energy: 'high', brightness: 'high', complexity: 'low', color_warmth: 'high' } },
  { text: 'MC Escher impossible architecture, tessellations, mind-bending perspective', axes: { realism: 'low', complexity: 'high', energy: 'low', brightness: 'low', color_warmth: 'low' } },
...
const MOOD_POOL: TaggedOption[] = [
  ...
  { text: 'quietly awe-inspiring', axes: { energy: 'low', color_warmth: 'low', complexity: 'high' } },
  { text: 'still and crystalline', axes: { energy: 'low', color_warmth: 'low', brightness: 'high' } },
  ...
  { text: 'magical and enchanted', axes: { energy: 'low', brightness: 'high', complexity: 'high' } },
  ...
  { text: 'haunting and melancholic', axes: { brightness: 'low', energy: 'low', color_warmth: 'low' } },
  { text: 'spooky but cute', axes: { brightness: 'low', energy: 'low', color_warmth: 'high' } },
  // ── Playful (mid energy) ──
  { text: 'playful and whimsical', axes: { energy: 'low', brightness: 'high', color_warmth: 'high' } },
  { text: 'mischievous and sneaky', axes: { energy: 'high', brightness: 'low', color_warmth: 'low' } },
  { text: 'silly and absurd', axes: { energy: 'high', brightness: 'high', color_warmth: 'high' } },
  { text: 'cheeky and irreverent', axes: { energy: 'high', brightness: 'high', complexity: 'low' } },
  // ── Intense + Warm ──
  { text: 'epic and grandiose', axes: { energy: 'high', complexity: 'high', color_warmth: 'high' } },
  { text: 'triumphant and heroic', axes: { energy: 'high', brightness: 'high', color_warmth: 'high' } },
  { text: 'passionate and fiery', axes: { energy: 'high', color_warmth: 'high', brightness: 'low' } },
  { text: 'luxurious and opulent', axes: { complexity: 'high', color_warmth: 'high', brightness: 'high' } },
  // ── Intense + Cool ──
  { text: 'mysterious and suspenseful', axes: { energy: 'high', brightness: 'low', color_warmth: 'low' } },
  { text: 'electric and charged', axes: { energy: 'high', color_warmth: 'low', brightness: 'high' } },
  { text: 'surreal and otherworldly', axes: { energy: 'high', complexity: 'high', color_warmth: 'low' } },
  { text: 'chaotic and energetic', axes: { energy: 'high', color_warmth: 'low' } },
  // ── Intense + Dark ──
  { text: 'dramatic and cinematic', axes: { energy: 'high', brightness: 'low', complexity: 'high' } },
  { text: 'ominous and foreboding', axes: { energy: 'high', brightness: 'low', color_warmth: 'low' } },
  // ── Complex / Simple ──
...
const DREAM_SUBJECTS = [
  ...
  'a fierce warrior in ornate battle armor', 'a rock star mid-guitar-solo, leather and chains',
  'a shadowy figure in a slinky evening outfit, noir lighting', 'a blacksmith hammering at a glowing forge',
  'a haute couture figure strutting through a fantasy world', 'a brooding vampire in a velvet cloak',
  'a sea creature lounging on sun-warmed rocks', 'a gladiator raising a sword to the crowd',
  'a sorcerer in flowing robes channeling arcane power', 'an elven archer poised to fire, wind in their hair',
  'a dancer mid-leap, fabric trailing like wings', 'a cyberpunk hacker in sleek gear and neon visor',
...
const ALWAYS_EXPAND = new Set([
  'gaming', 'movies', 'music', 'geek', 'sports', 'travel', 'pride', 'fashion',
  'animals', 'nature', 'ocean', 'dark', 'cute', 'fantasy', 'sci_fi', 'space',
]);
...
function filterPool(pool: TaggedOption[], rolledAxes: Record<string, 'high' | 'low'>, chaos: number = 0.5): string {
  ...
  if (Math.random() < 0.1 + chaos * 0.3) {
    ...
    const wildcardPool = chaos >= 0.5
      ? scored
      : scored.slice(0, Math.ceil(scored.length / 2));
...
const HIGH_COMPLEXITY_SIGNALS = new Set([
  // interests
  'architecture', 'fantasy', 'sci_fi', 'abstract', 'geek',
  // personality tags
  'elegant', 'mysterious', 'fierce', 'bold', 'futuristic',
  // eras
  'victorian', 'steampunk', 'art_deco', 'medieval',
]);
const LOW_COMPLEXITY_SIGNALS = new Set([
  // interests
  'cute', 'food', 'animals',
  // personality tags
  'cozy', 'gentle', 'peaceful', 'playful',
  // eras
  'modern', 'retro',
]);
